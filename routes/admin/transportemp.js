var transportemp = require('../../controllers/transportemp');
...
router.post('/', oauth.oauth.authorise(), upload.array(), function (req, res) {
  req.roleAccess = {model:'transportemp', action:'view'};
    auth.checkPermissions(req, function(isPermission){
        if (isPermission.status === true) {
            transportemp.list(req, function(result){
                res.send(result);
            });
        } else {
...
router.post('/save', oauth.oauth.authorise(), function (req, res) {
    uploadFile(req, res, function (err) {
        ...
        var count = 1;
        req.roleAccess = {model:'transportemp', action:['add', 'edit']};
        auth.checkPermissions(req, function(isPermission){
            if (isPermission.status === true) {
                async.forEach(req.files, function (up_files, callback) {
                  ...
                  count++;
                    ...
                    transportemp.save(data, function(result){
                        res.send(result);
                    });
                });
            } else {
...
router.post('/add', oauth.oauth.authorise(), upload.array(), function (req, res) {
    ...
    req.roleAccess = {model:'transportemp', action:'add'};
    auth.checkPermissions(req, function(isPermission){
        if (isPermission.status === true) {
            res.send(data);
        } else {
...
router.post('/edit/:id', oauth.oauth.authorise(), upload.array(), function (req, res) {
    ...
    req.roleAccess = {model:'transportemp', action:'edit'};
    auth.checkPermissions(req, function(isPermission){
        if (isPermission.status === true) {
            transportemp.getById(data, function(result){
                res.send(result);
            });
        } else {
            res.send(isPermission);
...
router.post('/status/:id/:status', oauth.oauth.authorise(), upload.array(), function (req, res) {
    ...
    req.roleAccess = {model:'transportemp', action:'status'};
    auth.checkPermissions(req, function(isPermission){
        if (isPermission.status === true) {
            transportemp.status(data, function(result){
                res.send(result);
            });
        } else {
            res.send(isPermission);
