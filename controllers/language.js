...
var i18n = new I18n({locales: ['en', 'fr', 'ar']});
...
function Language() {
	...
	*/
	...
	/*
	 * get All Language Alphabetically
	*/
	this.getAllLanguageAlphabetically = function(req, res) {
		orderBy = 'name ASC';
		models.language.findAll({where:{id:{$ne:1}, is_active:1}, order: orderBy}).then(function(data){
			res(data);
		}).catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
	...
	*/
	 ...
	 * User Languages
	*/
	this.getUserLanguagesWeb = function(req, res) {
		if (req.masterId == 1 ) {
			...
			models.user.find(
				{
					attributes:['secondary_lang'],
					where:{
						masterId:req.masterId
					}
				}
			).then(function(institute){
				if (institute.secondary_lang !== 0) {
				langId = {$in:[1, institute.secondary_lang]};
				}
