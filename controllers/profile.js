...
function Profile() {
					...
					if (typeof req.id !== 'undefined' && req.id !== '') {
						...
						models.user
							...
							.then(function(data) {
								req.user_detail.languageId = req.langId;
								models.userdetail
									.find({ where: { userId: req.id, languageId: req.langId } })
									.then(function(resultData) {
										if (resultData !== null) {
											req.user_detail.id = resultData.id;
											models.userdetail
												.update(req.user_detail, {
													where: {
														id: resultData.id,
														userId: req.id,
	...
	*/
						...
						models.user
							...
							.then(function(data) {
								res({
									...
									data: data,
									defaultSessionId: req.defaultSessionId,
								});
							})
							.catch(() =>
								res({
	 ...
	 * change bank details
	*/
	this.changeBankDetails = function(req, res) {
		var instituteObj = {};

		instituteObj['bank_name'] = req['institute[bank_name]'];
		instituteObj['ifsc_code'] = req['institute[ifsc_code]'];
		instituteObj['bank_branch'] = req['institute[bank_branch]'];
		instituteObj['account_no'] = req['institute[account_no]'];
		instituteObj['pan_no'] = req['institute[pan_no]'];
		instituteObj['bank_challan_charges'] =
			req['institute[bank_challan_charges]'];

		var institute = models.institute.build(instituteObj);
		var errors = [];
		// an example using an object instead of an array
		async.parallel(
			[
			...
			function(err, errors) {
				...
				if (uniqueError.length === 0) {
					if (typeof req.id !== 'undefined' && req.id !== '') {
						models.institute
							.update(instituteObj, { where: { userId: req.id } })
							.then(function(data) {
								res({
									status: true,
									message: language.lang({
	...
	*/
	this.getById = function(req, res) {
		...
		isWhere.countrydetailInstitute = language.buildLanguageQuery(
			isWhere.countrydetailInstitute,
			req.langId,
			'`institute.country`.`id`',
			models.countrydetail,
			'countryId'
		);
		isWhere.statedetailInstitute = language.buildLanguageQuery(
			isWhere.statedetailInstitute,
			req.langId,
			'`institute.state`.`id`',
			models.statedetail,
			'stateId'
		...
		isWhere.boarddetail = language.buildLanguageQuery(
			isWhere.boarddetail,
			req.langId,
			'`student.studentrecord.bcsmap.board`.`id`',
			models.boarddetail,
			'boardId'
		);
		isWhere.classesdetail = language.buildLanguageQuery(
			...
			req.langId,
			'`student.studentrecord.bcsmap.class`.`id`',
		...
		isWhere.sectiondetail = language.buildLanguageQuery(
			...
			req.langId,
			'`student.studentrecord.bcsmap.section`.`id`',
		...
		if (req.user_type === 'teacher') {
			models.user
									...
									include: [
										{
											...
											where: isWhere.countrydetailTeacher
										}
									]
								},
								{
									...
									include: [
										{
											...
											where: isWhere.statedetailTeacher
										}
									]
								},
								{
									...
									include: [
										{
											...
											where: isWhere.citydetailTeacher
										}
									]
								}
							]
				...
				.then(function(data) {
					models.institute
						...
						.then(institute => {
							data = data.toJSON();
							data.institute_name = institute.institutedetails[0].name;
			...
			models.user
				.find({
							...
							include: [
								{ model: models.studentdetail, where: isWhere.countrydetail },
								{
									...
									include: [
										{
											model: models.countrydetail,
											where: isWhere.countrydetailStudent
										}
									]
								},
								{
									...
									include: [
										{
											model: models.statedetail,
											where: isWhere.statedetailStudent
										}
									]
								},
								{
									...
									include: [
										{
											model: models.citydetail,
											where: isWhere.citydetailStudent
										}
									]
								},
								{
									...
									where: {
										academicSessionId: {
											$eq: models.sequelize.literal(
												'(SELECT `defaultSessionId` FROM `users` WHERE `id` = `student`.`masterId`)'
											)
										}
										/*record_status:1,
									$or: [
					...
					where: { id: req.id },
					order: [[models.student, models.studentrecord, 'id', 'DESC']],
					limit: 1,
					subQuery: false
				})
				.then(function(data) {
							...
							include: [
								...
								},
								{
									...
									include: [
										{
											model: models.countrydetail,
											where: isWhere.countrydetailInstitute
										}
									]
								},
								{
									...
									include: [
										{
											model: models.statedetail,
											where: isWhere.statedetailInstitute
										}
									]
								},
								{
									...
									include: [
										{
											model: models.citydetail,
											where: isWhere.citydetailInstitute
										}
									]
								}
							]
			...
			models.user
				.find({
					include: [{ model: models.userdetail, where: isWhere.userdetail }],
					where: {
						id: req.id
					}
				})
		...
		if (req.user_type === 'institute') {
			...
			models.institute
				.findOne({
					...
					attributes: ['id', 'institute_image', 'institute_logo']
				})
				.then(
					institute => {
						res({
							...
							userdetails: [
								{
									fullname: institute.institutedetails[0].name,
									id: institute.institutedetails[0].id
								}
							],
	...
	this.saveUserProfile = function(req, res) {
		if (req.user_type === 'institute') {
			var user = models.user.build({ mobile: req.mobile, email: req.email });
			Promise.all([user.validate()])
				...
				.then(errors => {
					if (errors.length === 0) {
						var updates = [
							models.institute
								.findById(req.id, { attributes: ['id', 'userId'] })
								.then(institute => {
			...
			let user = models.user.build(req),
				userdetail = models.userdetail.build(req.userdetail);
			Promise.all([user.validate(), userdetail.validate()])
				.then(err => {
					if (err[0]) {
						if (err[1]) return err[0].errors.concat(err[1].errors);
						else return err[0].errors;
					} else {
						if (err[1]) return err[1].errors;
						else return null;
					}
				})
				.then(errors => {
					...
					return Promise.all([
						...
						models.userdetail
							.findOne({ where: { id: req.userdetail.id } })
							.then(userdetail => {
						...
						req.institute_logo &&
							models.institute.update(
								{
									institute_logo: req.institute_logo
								},
								{
									where: {
										userId: req.id
						...
						.then(([,userdetail]) =>
							res({
								...
								fullname: req.userdetail.fullname,
								...
								institute_logo: req.institute_logo ? path.join(
									'public/uploads',
									req.institute_logo.substring(tmpDir.length)
								) : ''
							})
						)
						.catch(() =>
