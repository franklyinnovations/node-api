					...
					models.doctorprofile.update(req, {
						...
						models.doctorprofiledetail.find({
							...
							if (resultData !== null) {
								...
								console.log(req.doctor_profile_details)
		...
		models.doctorprofile.find({
			...
			include: [
				...
				{model: models.city,attributes:['id'],include:[
					{model: models.citydetail,attributes:['name'],where:language.buildLanguageQuery({}, req.langId, '`city`.`id`', models.citydetail, 'citiyId')}
				]},
				{
					model: models.doctorprofiledetail,
					where: language.buildLanguageQuery({}, req.langId, '`doctorprofile`.`id`', models.doctorprofiledetail, 'doctorProfileId')
				...
				{
					model: models.doctorregistration,
					attributes: ['council_registration_number', 'year_of_registration', 'reg_proof'],
					include: [{
						model: models.doctorregistrationdetail,
						attributes: ['council_name'],
						where: language.buildLanguageQuery({}, req.langId, '`doctorexperience`.`id`', models.doctorregistrationdetail, 'doctorRegistrationId'),
						required: false
					}],
					required: false
				},
		...
		models.doctorprofile.findOne({
			...
			if(data){
				...
				Promise.all([
					...
					new Promise((resolve) => tagtype.listByType({body: {id: utils.getAllTagTypeId()['ServiceTagId']}, where: {'$or': [{userId: req.userId}, {is_active: 1, is_approved: 1}]}}, (result) => resolve(result))),
					new Promise((resolve) => tagtype.listByType({body: {id: utils.getAllTagTypeId()['EducationQualificationTagId']}, where: {'$or': [{userId: req.userId}, {is_active: 1, is_approved: 1}]}}, (result) => resolve(result))),
					new Promise((resolve) => tagtype.listByType({body: {id: utils.getAllTagTypeId()['SpecializationTagId']}}, (result) => resolve(result))),
					new Promise((resolve) => tagtype.listByType({body: {id: utils.getAllTagTypeId()['MembershipsTagId']}, where: {'$or': [{userId: req.userId}, {is_active: 1, is_approved: 1}]}}, (result) => resolve(result)))
				]).then(
	...
	this.DoctorBasicAndSpecInfo = function(req, res) {
		...
		models.doctorprofile.find({
			...
			include: [{
					model: models.doctorprofiledetail,
					attributes: ["name", "address_line_1"],
					where: language.buildLanguageQuery({}, req.body.langId, '`doctorprofiles`.`id`', models.doctorprofiledetail, 'doctorProfileId')
				},
				{
					model: models.doctortags,
					required: false,
					...
					include: [
						{
							...
							include: [
								{
									model: models.tagdetail, required: false,
									where: language.buildLanguageQuery({}, req.body.langId, '`tag`.`id`', models.tagdetail, 'tagId'),
									attributes: ["title"]
								}
							]
						}
