...
*/
    ...
    */
        ...
        models.MedicalRecordItem.belongsTo(models.tag,{foreignKey: 'medical_record_type'});
        models.tag.hasMany(models.tagdetail);

        models.MedicalRecord.findAll({
         attributes:['id','patientId','date'],
          include: [
            { model: models.MedicalRecordDetail, where: where.MedicalRecordDetail,attributes:['id','languageId','title','patient_name'],required:false},
            ...
            { model: models.MedicalRecordItem,include:[{model:models.tag,include:[{model:models.tagdetail,attributes: ['id','title']}],attributes: ['id']}],attributes: ['id', 'medical_record_type'], required:false},
           ...
           async.forEachOf(result,function(entry,index,callback) {
            ...
            async.forEachOf(entry.MedicalRecordItems,function(entrynew,index1,callback1) {
                ...
                callback1(null, result1);
                    ...
                    console.log(result1[index].dataValues.MedicalRecordItems);
                    ...
                    res({
                        status: true,
                        message:language.lang({key: "item_list", lang: reqData.lang}),
                        data: result1,
                      });
    ...
    this.save = function (req, res) {
        console.log(req)
        var medicalRecordDetailHasOne = models.MedicalRecord.hasOne(models.MedicalRecordDetail, {as: 'medical_record_details'});
        var medicalRecordItemlHasOne = models.MedicalRecord.hasOne(models.MedicalRecordItem, {as: 'medical_record_items'});
        
        if(typeof req.title != 'undefined'){
            req.medical_record_details={title:req.title,languageId:req.langId,patient_name:req.patient_name};
        } else {
            req.medical_record_details={title:language.lang({key: "Added by you",lang: req.lang}),languageId:req.langId,patient_name:req.patient_name};
        }
        req.medical_record_items={img:req.img,medical_record_type:req.medical_record_type};
        ...
        async.parallel([
            ...
            if (uniqueError.length === 0) {
                ...
                if (typeof req.patientId !== 'undefined' && req.patientId !== '') {
                    models.MedicalRecord.find({where: {patientId: req.patientId,date:req.date}}).then(function (medicalData) {
                        if (medicalData !== null) {
                                /*
                                medical_record_details create and update according to languageId
                                */
                                models.MedicalRecordDetail.find({where: {medicalRecordId: medicalData.id, languageId: req.langId}}).then(function (resultData) {
                                    if (resultData !== null) {
                                        models.MedicalRecordDetail.update(req.medical_record_details, {where: {medicalRecordId: medicalData.id, languageId: req.langId}}).then(function (updateData) {
                                            res({status: true, message: language.lang({key: "addedSuccessfully", lang: req.lang}), data: updateData});
                                        }).catch(() => res({status: false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
                                    } else {
                                        ...
                                        models.MedicalRecordDetail.create(req.medical_record_details).then(function (medicalRecordDetailss) {
                                            res({status: true, message: language.lang({key: "addedSuccessfully", lang: req.lang}), data: medicalRecordDetailss});
                                        }).catch(() => res({status: false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
                                    }
                                }).catch(() => res({status: false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
                                /*
                               ...
                               if(typeof req.medical_record_type != 'undefined'){
                                    ...
                                    models.MedicalRecordItem.create(req.medical_record_items).then(function (recordItem) {
                                        res({status: true, message: language.lang({key: "addedSuccessfully", lang: req.lang}), data: recordItem});
                                    }).catch(() => res({status: false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
                                }
                        } else {
                            models.MedicalRecord.create(req, {include: [medicalRecordDetailHasOne,medicalRecordItemlHasOne], individualHooks: true}).then(function (patientData) {
    ...
    this.delete = function (req, res) {
        ...
        models.MedicalRecordItem.destroy({where: {medicalRecordId: req.id}});
        models.MedicalRecordDetail.destroy({where: {medicalRecordId: req.id}})
        models.MedicalRecord.destroy({where: {id: req.id}})
        .then(data => res({
            status: true
    ...
    this.deleteItem = function (req, res) {
        models.MedicalRecordItem.destroy({where: {id: req.id}})
        .then(data => res({
            status: true
            , message:language.lang({key:"deletedSuccessfully", lang:req.lang})
            , data: data
    ...
    where={};
        ...
        models.MedicalRecord.find({
          ...
          include: [
            { model: models.MedicalRecordDetail, where: where.MedicalRecordDetail,required:false},
            { model: models.MedicalRecordItem,include:[{model:models.tag,include:[models.tagdetail]}], required:false},
