...
function Patient() {
  ...
  this.list = function (req, res) {
    ...
    models.hospital.findAndCountAll({
      include: [
        { model: models.hospitaldetail, where: where.hospitaldetail},
        { model: models.hospitalfile, where: where.hospitalfile},
        { model: models.country,include: [{model: models.countrydetail, where:where.countrydetail}]},
        { model: models.state,include: [{model: models.statedetail, where:where.statedetail}]},
        { model: models.city,include: [{model: models.citydetail, where:where.citydetail}]}
      ],
      distinct: true,
...
this.questionlist = function (req, res) {
  ...
  // distinct: true,     
  attributes:['id','problem_title','description','createdAt'],
  ...
  totalData: result.count,        
  pageCount: Math.ceil(result.count / pageSize),         
  pageLimit: pageSize,         currentPage:page
  });     
  })     
  .catch(() => res({ 
  status:false,      
...
*/
  ...
  this.questiondetail = function(req, res) {
    ...
    where.doctorprofiledetail = language.buildLanguageQuery(
        where.doctorprofiledetail, req.langId, '`doctorprofile`.`id`', models.doctorprofiledetail, 'doctorProfileId'
    );
    models.patientquestion.find({
      ...
      include: [{
        ...
        include: [{
          ...
          attributes: [
            'id',
            'doctor_profile_pic', [models.sequelize.literal('(SELECT (MAX(duration_to)-MIN(duration_from)) FROM doctor_experiences WHERE doctor_experiences.doctorProfileId = `question_answers.doctorprofile`.`id`)'), 'total_exp']
          ],
          required: false,
          include: [{
  ...
  */
  this.save = function(req, res){
    var patientHasOne = models.patient.hasOne(models.patientdetail, {as: 'patient_detail'});
    ...
    async.parallel([
      ...
      if (uniqueError.length === 0) {
        if (typeof req.userId !== 'undefined' && req.userId !== '') {
          models.patient.find({where:{userId:req.userId}}).then(function(patientData){
              
            if (patientData !==null) {

                      models.patient.update(req,{where: {id:patientData.id}, individualHooks: true}).then(function(data){
                        models.patientdetail.find({where:{patientId:patientData.id,languageId:req.langId}}).then(function(resultData){
                          ...
                          if (resultData !==null) {
                            models.patientdetail.update(req, {where:{patientId:patientData.id,languageId:req.langId}}).then(function(){
                            ...
                            res({status:true, message:language.lang({key:"addedSuccessfully", lang:req.langId}), data:patientDetailss});
                            }).catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
                          }
                        }).catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
                      }).catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
            ...
            models.patient.create(req, {include: [patientHasOne], individualHooks: true}).then(function(patientData){

              res({status:true, message:language.lang({key:"addedSuccessfully", lang:patientData.languageId}), data:patientData});
            })//.catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
          }
          })
      }else{
...
/*
  ...
  */
  this.addtag = function(req, res){
    ...
    async.parallel([
      ...
      if (uniqueError.length === 0) {
        if (typeof req.patientId !== 'undefined' && req.patientId !== '') {
            models.patienttag.destroy({where: {patientId: req.patientId,tagtypeId:req.tagtypeId}});
            console.log(req);
            ...
            req.tagId.split(",").map(function (value) {
              ...
              models.patienttag.create(req).then(function(patienttags){
                res({status:true, message:language.lang({key:"addedSuccessfully", lang:req.langId}), data:patienttags});
              }).catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
            }else{
              res({status:true, message:language.lang({key:"addedSuccessfully", lang:req.langId}), data:{}});
            }
            })
      } else if (typeof req.userId !== 'undefined' && req.userId !== '') {
          
            models.patient.find({where:{userId:req.userId}, attributes: ['id']}).then(function(userData){
                if (userData === null) {
                    res({status:false, message:language.lang({key:"invalid_detail", lang:req.lang})});
                    
                } else {
                    
                    models.patienttag.destroy({where: {patientId: userData.id,tagtypeId:req.tagtypeId}});
                    req.tagId.split(",").map(function (value) {
                      req.tagId = value;
                      if(req.tagId!=''){
                      models.patienttag.create(req).then(function(patienttags){
  ...
  */
  this.status = function(req, res) {
    models.hospital.update(req,{where:{id:req.id}}).then(function(data){
    ...
    models.hospital.find({
      include: [
        {model: models.hospitaldetail, where:isWhere.hospitaldetail},
          {model: models.hospitalfile, where:isWhere.hospitalfile,required:false},
          {model: models.hospital_timings, where:isWhere.hospital_timings,required:false},
          {model: models.hospitalservice, where:isWhere.hospitalservice,required:false},
          { model: models.hospitalaward,include: [{model: models.hospitalawarddetail, where:isWhere.hospitalawarddetail,required:false}],required:false},
         ...
         state.getAllState(req, function(states){
          city.getAllCity(req, function(cities){
            doctor.listDoctor(req, function(doctors){
                res({data:data,countries:countries,states:states,cities:cities,doctors:doctors});
            });
          })
          })
         });
...
/*
    ...
    this.getByLocation = function(req, res) {
      ...
      models.doctorprofile.belongsTo(models.city,{foreignKey:'cityId',targetKey: 'id'});
      ...
      models.hospital_doctors.belongsTo(models.hospital,{foreignKey:'hospitalId',targetKey: 'id'});

      models.hospital.hasMany(models.contactinformation,{foreignKey:'key',sourceKey:'id'});

      models.doctorprofile.hasMany(models.doctortags);
      models.doctortags.belongsTo(models.tag,{foreignKey:'tagId',targetKey: 'id'});
      models.tag.hasMany(models.tagdetail);
      ...
      models.hospital.hasMany(models.hospitaldetail)
      models.hospital.hasMany(models.hospitalfile);
      models.hospital.belongsTo(models.city,{foreignKey:'cityId',targetKey: 'id'});
      models.hospital.belongsTo(models.state,{foreignKey:'stateId',targetKey: 'id'});
      models.hospital.belongsTo(models.country,{foreignKey:'countryId',targetKey: 'id'});
      ...
      async.parallel({
        doctor_ids:function(callback){
            models.doctortags.find({
              attributes:[[models.sequelize.fn('GROUP_CONCAT', models.sequelize.col('doctorProfileId')), 'doctorProfileId']],required:false,where:{tagId:req.tagId}
        ...
        hospital_ids:function(callback){
          models.hospitalservice.find({
            attributes:[[models.sequelize.fn('GROUP_CONCAT', models.sequelize.col('hospitalId')), 'hospitalId']],required:false,where:{tagId:req.tagId}
    ...
    //doctor ids by tagIds
     ...
     whereHospital={is_live:1,is_active:1}
    isCondition = {status:1}
    if('undefined'!==typeof req.consultation_charge){
      var feeArray = req.consultation_charge.split("-");
      isCondition = {status:1,consultation_charge:{$between:feeArray}}
    }
    if('undefined'!==typeof req.online_booking){
      isCondition = {status:1,available_on_req:0}
      whereHospital = {active_schedule:1,is_live:1,is_active:1}
    }


    if('undefined'!==typeof req.consultation_charge && 'undefined'!==typeof req.online_booking){
      var feeArray = req.consultation_charge.split("-");
      isCondition = {status:1,consultation_charge:{$between:feeArray},available_on_req:0}
      whereHospital = {active_schedule:1,is_live:1,is_active:1}
    }
          ...
          async.parallel({
              ...
              doctors: function (callback) {
                models.doctorprofile.findAll({
                  ...
                  include: [
                      {model: models.doctorprofiledetail,attributes:['name'], where:isWhere.doctorprofiledetail },
                     // {model: models.contactinformation,attributes:['value'],where:{type:'mobile',is_primary:1}, required: false},
                      {model: models.hospital_doctors,attributes:['consultation_charge','available_on_req','id'],where:isCondition,
                        include:[
                          {model: models.hospital,attributes:['id','active_schedule'],include:[
                            {model:models.hospitaldetail,attributes:['hospital_name']},
                            {model: models.contactinformation,attributes:['value'],where:{type:'mobile',is_primary:1}, required: false}
                          ],
                          where:whereHospital
                        },
                      ...
                      {model: models.doctoreducation,attributes:['tagtypeId'],
                          include: [ 
                            { model: models.tag,attributes:['id'],required:false,include:[
                      ...
                      {model: models.doctortags,attributes:['tagId'],
                      include: [ 
                        { model: models.tag,attributes:['id'],required:false,where:{tagtypeId:2},
                        include:[
                          {model: models.tagdetail,attributes:['title'],required:false, where: language.buildLanguageQuery({}, req.langId, '`tag`.`id`', models.tagdetail, 'tagId')}
                        ]},
                     ...
                     /* order: [
                      ...
                      {model: models.doctorfile,attributes:['doctor_files'],where:{document_type:'public_photos',file_type:'image'}, required: false},
              ...
              hospitals: function (callback) {
                models.hospital.findAll({
                  attributes:[
                      ...
                      'is_live',
                      'is_active',
                      [models.sequelize.literal('(SELECT IFNULL(ROUND(AVG(doctor_feedbacks.rating)),0) FROM doctor_feedbacks WHERE doctor_feedbacks.hospitalId = hospital.id)'), 'avg_rating']
                  ],
                  include: [
                      {model: models.hospitaldetail,attributes:['hospital_name','address'], where: isWhere.hospitaldetail},
                    
                      {model: models.hospitalfile,attributes:['hospital_files'],where:{document_type:'public_photos',file_type:'image'}, required: false},
                      ...
                      {model: models.country,attributes:['id'],include:[
                        {model:models.countrydetail,attributes:['name'],where: language.buildLanguageQuery({}, req.langId, '`ccountry`.`id`', models.countrydetail, 'countryId')}
                      ], required: false},
                      {model: models.contactinformation,attributes:['value'],where:{type:'mobile',is_primary:1}, required: false}
                    ],
                  where:{
              ...
              res({
                  status:true,
                  message:language.lang({key: "hospitals and doctors list by location", lang: req.lang}),
                  data:{doctors: result.doctors,hospitals: result.hospitals},
  ...
  this.patientdetail = function(req, res) {
    ...
    where.tagdetail = language.buildLanguageQuery(
      where.tagdetail, req.langId, '`patienttag.tag`.`id`', models.tagdetail, 'tagId'
    );
  ...
  this.makeFavouriteDoctor=function(req,res){
    models.favouritedoctor.find({
      attributes: [[models.sequelize.fn('COUNT', models.sequelize.col('id')), 'is_favourite']],
      where:{patientId:req.patientId,doctorProfileId:req.doctorProfileId}
    }).then(function(data){
        if(data.dataValues.is_favourite==0){
          req.is_favourite=1
          models.favouritedoctor.create(req).then(function(createdata){
            res({status:true,data:{is_favourite:1},message:language.lang({key:"startedSuccessfully", lang:req.lang})})
          })
        }else{
          models.favouritedoctor.destroy({
            where:{patientId:req.patientId,doctorProfileId:req.doctorProfileId}
        }).then(function(){
          res({status:true,data:{is_favourite:0},message:language.lang({key:"unstartedSuccessfully", lang:req.lang})})
        })
  ...
  this.CheckFavouriteDoctor = function(req , res){
    models.favouritedoctor.find({
      ...
      res({status:true,is_favourite:data.dataValues.is_favourite})
    })
  }

  this.myDoctors = function(req , res){
    models.favouritedoctor.belongsTo(models.doctorprofile,{foreignKey:'doctorProfileId',targetKey:'id'})
    models.doctorprofile.hasMany(models.doctorprofiledetail);
    models.doctorprofile.hasMany(models.doctortags);
    models.doctorprofile.hasMany(models.doctoreducation);
    models.doctortags.belongsTo(models.tag,{foreignKey:'tagId',targetKey:'id'});
    ...
    models.favouritedoctor.findAll({
      ...
      include:[
        {model:models.doctorprofile,attributes:[
          'id',
          'salutation',
          'doctor_profile_pic',
          [models.sequelize.literal('(SELECT (MAX(duration_to)-MIN(duration_from)) FROM doctor_experiences WHERE doctor_experiences.doctorProfileId = doctorprofile.id)'),'total_exp'],
        ...
        include:[
          {model: models.doctorprofiledetail,attributes:['name'], where: language.buildLanguageQuery({}, req.langId, '`doctorprofile`.`id`', models.doctorprofiledetail, 'doctorProfileId')},
          {model: models.doctortags, 
            include:[
      ...
      {model: models.hospital_doctors,attributes:['consultation_charge'],required:false}
        ]
        ,required:false
      },
      ],
  ...
  this.patientChangeNotificationSettings = function(req, res) {
    var notificationColumns = {is_chat_notification: 'is_chat_notification', is_appointment_notification: 'is_appointment_notification'}
    if(req.type === 'all') {
      models.patient.find({attributes: ['userId'], where: {id: req.patientId}}).then(function(patientdata) {
        models.user.update({is_notification: req.value}, {where: {id: patientdata.userId}}).then(function(data) {
          models.patient.update({
            is_chat_notification: req.value, 
            is_appointment_notification: req.value
          }, {where: {id: req.patientId}}).then(function(data) {
            res({
              status: true,
      ...
      models.patient.update({[notificationColumns[req.type]]: req.value}, {where: {id: req.patientId}}).then(function(result) {
        res({
          status: true,
          message: language.lang({key:"success",lang:req.lang})
        })
