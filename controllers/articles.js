...
function article() {
	...
	this.getList = function(req, res) {
		...
		*/
		...
		isWhere.articledetail = language.buildLanguageQuery(isWhere.articledetail, reqData.langId, '`articles`.`id`', models.articledetail, 'articleId');
	...
	this.getDetail = function(req, res) {
		...
		models.patient.findOne({attributes: ["id"], where: {userId: reqData.userId}}).then(function(profileData) {
			models.article.findOne({
				...
				include: [
					{model: models.articledetail, attributes: ["title", "body"], where: language.buildLanguageQuery({}, reqData.langId, '`articles`.`id`', models.articledetail, 'articleId')},
					{
						model: models.doctorprofile, 
						attributes: ["id","salutation","doctor_profile_pic"], 
						include: [
							{
								model: models.doctorprofiledetail, attributes: ["name"], 
								where: language.buildLanguageQuery({}, reqData.langId, '`user`.`id`', models.doctorprofiledetail, 'doctorProfileId')
							}, {
								model: models.doctortags,
								attributes: ["tagId"],
								...
								include: [
									{
										model: models.tagdetail, attributes: ["title"], where: language.buildLanguageQuery({}, reqData.langId, '`tagdetail`.`tagId`', models.doctortags, 'tagId')
									}
								],
								required: false
							}
				...
				where:{ id: reqData.id, status: 1, is_active: 1 }
			}).then(function(data){
				models.article.findAll({
					attributes:[
                                            ...
                                            "article_image", 
                                            "createdAt",
                                            [models.sequelize.literal('(SELECT COUNT(article_likes.id) FROM article_likes WHERE article_likes.articleId = article.id)'), 'total_likes']],
					where: {keyId: data.keyId, id: {$ne: data.id}, status: 1, is_active: 1},
					include: [
						{model: models.articledetail, attributes: ["title", "body"], where: language.buildLanguageQuery({}, reqData.langId, '`articles`.`id`', models.articledetail, 'articleId')},
						{
					...
					res({status: true, message: language.lang({key: "Article detail", lang: req.lang}), data: data, other_articles: otherArticles});
				}).catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
			}).catch(() => res({status:false, error: true, error_description: language.lang({key: "Internal Error", lang: req.lang}), url: true}));
		})
	};
				...
				if(associateProfileData) {
					models.starredarticle.find({
						...
						if(starredArticles.articleIds) {
							...
							models.article.hasMany(models.starredarticle, {foreignKey: 'articleId', sourceKey: 'id'});
							models.article.findAndCountAll({
								attributes: ["id", "keyId", "article_image",[models.sequelize.literal('(SELECT COUNT(article_likes.id) FROM article_likes WHERE article_likes.articleId = article.id)'), 'total_likes']],
								include: [
									{model: models.articledetail, attributes: ["title", "body"], where: language.buildLanguageQuery({}, req.langId, '`articles`.`id`', models.articledetail, 'articleId')},
									{
										...
										include: [
											...
											},
											{
												...
												include: [
													{
														model: models.tagdetail, attributes: ["title"], where: language.buildLanguageQuery({}, req.langId, '`tagdetail`.`tagId`', models.doctortags, 'tagId')
													}
												],
												required: false
											}
...
this.most_like_article=function(req,res){
	models.sequelize.query(
	  'select newTb.most_like,newTb.artical_id,articles.article_image,articles.createdAt,article_details.title,article_details.body,doctor_profile_details.name,doctor_profiles.salutation from (select count(al.articleId) as most_like,articleId as artical_id from article_likes al group by articleId) as newTb INNER JOIN articles on(articles.id=newTb.artical_id) INNER JOIN article_details on(article_details.articleId=articles.id) INNER JOIN doctor_profiles on(doctor_profiles.id=articles.keyId) INNER JOIN doctor_profile_details on(doctor_profile_details.doctorProfileId=doctor_profiles.id) where article_details.languageId='+req.langId+' order by newTb.most_like desc limit 1', { type: models.sequelize.QueryTypes.SELECT }
	).then(function(data){
